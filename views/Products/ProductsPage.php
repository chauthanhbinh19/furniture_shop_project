        ...
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
                <div class="d-flex justify-content-between align-items-center w-100">
                    ...
                    <a href="/admin/insert-products" class="btn btn-primary">
                        <i class="cil-user-follow"></i> Add product
                    </a>
                </div>
            </div>
        </div>
                        ...
                        <?php foreach ($data['products'] as $product): ?>
                            <tr>
                                ...
                                <td class="text-center"><?php
                                                        $avatar = !empty($product['image']) ? htmlspecialchars($product['image']) : 'no-avatar.png';
                                                        ?>
                                    <img src="../../public/assets/avatars/<?= $avatar ?>" alt="Avatar" class="rounded-circle" width="40" height="40" style="margin-right:5px;">
                                </td>
                                <td class="text-center"><?= htmlspecialchars($product['full_name']) ?></td>
                                <td class="text-center"><?= htmlspecialchars($product['email']) ?></td>
                                <td class="text-center"><?= htmlspecialchars($product['phone_number']) ?></td>
                                <td class="text-center"><?= htmlspecialchars($product['gender']) ?></td>
                                <td class="text-center">
                                    <?php if ($product['status'] === 'active'): ?>
                                        <span class="badge bg-success text-white">Active</span>
                                    <?php elseif ($product['status'] === 'unactive'): ?>
                                        <span class="badge bg-danger text-white">Unactive</span>
                                    <?php else: ?>
                                        <span class="badge bg-secondary text-white"><?= htmlspecialchars($product['status']) ?></span>
                                    <?php endif; ?>
                                </td>
                                <td class="text-center"><!-- Định dạng ngày tạo tài khoản -->
                                    <?= date('d-m-Y', strtotime($product['created_at'])) ?></td>
                                <td>
                                    <button class="btn btn-outline-warning" data-coreui-toggle="modal" data-coreui-target="#editproductModal<?= $product['id'] ?>">
                                        Edit
                                    </button>
                                    <a href="delete_product.php?id=<?= $product['id'] ?>" class="btn btn-outline-danger" onclick="return confirm('Are you sure?')">Delete</a>
                                    <!-- Include modal update ở đây -->
                                    <!-- <?php include __DIR__ . '/Updateproducts.php'; ?> -->
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
